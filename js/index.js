...
function facultyOf(n) {
  if (n < 2) {
    return 1;
  }
  return n * facultyOf(n - 1);
}
...
console.log(facultyOf(6));

const characters = fetch('https://rickandmortyapi.com/api/character').then(response => response.jsno());
...
function onlyAlive(characters) {
  return characters.filter(character => character.status === "Alive");
}

function nameArray(characters) {
  return characters.map(character => character.name);
}

function objectReturn(characters) {
...
console.log(onlyAlive(characters));
console.log(nameArray(characters));
console.log(objectReturn(characters));
